$(document).ready(function() {
  ...
  $('select[name=event]').change(function(){
    $('select[name=brewery] option[value!=""]').remove();
    event_id = $(this).val();
    request_url = '/lbtf/event/beer/get_breweries_at_event' + event_id + '/';
    $.ajax({
      url: request_url,
  ...
  $('select[name=brewery]').change(function(){
    $('select[name=beer] option[value!=""]').remove();
    brewery_id = $(this).val();
    request_url = '/lbtf/event/beer/get_beers_at_event_by_brewery' + brewery_id + '/';
    $.ajax({
      url: request_url,
      success: function(data){
        for (var key in data) {
  ...
  if ($('#event_beer_id').val().length !== 0 && $('#event_beer_id').val() !== '') {
    ...
    beer_name_request_url = '/lbtf/event/beer/get_beer_name' + event_beer_id + '/';
